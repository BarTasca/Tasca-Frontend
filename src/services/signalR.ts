...
export function registerStaffEventHandlers(
  handler: (payload: any, eventName: string) => void,
): () => void {
  const dev = readDevContext()
  if (dev.enabled) return () => {}
...
export async function joinTicketGroup(publicId: string): Promise<void> {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      rejoinCallbacks.add(async () => {
        try {
          await connection?.invoke('JoinTicketGroup', pid)
        } catch {}
      })
      return
    } catch (err: any) {
